...
#------------------------------------------------------------------------
# PARÁMETROS RECIBIDOS DESDE LA INTERFAZ
#------------------------------------------------------------------------
...
if len(sys.argv) >= 4:
    ANIO_OBJETIVO = int(sys.argv[2])
    MES_OBJETIVO = int(sys.argv[3])
...
def corregir_ium_por_cum(df):
    ...
    ruta_equivalencias = (
        BASE_DIR /
        'equivalencias_ium_cum.xlsx'
    )

    if not ruta_equivalencias.exists():
        print(
    ...
    for idx in df.index[mascara]:
        ...
        valor_nuevo = equivalencias_ium_cum.get(
            valor_anterior,
            valor_anterior
        )
...
def corregir_finalidades_por_diagnostico(df):
    ...
    df.loc[mascara_proc90_z,'finalidadTecnologiaSalud'] = '12'
...
if len(log_correcciones) > 0:
    ...
    nombre_factura = os.path.splitext(
        os.path.basename(ruta)
    )[0]

    ruta_log = os.path.join(
        r'C:\proyecto\log_errores',
        f'LOG_ERRORES_{nombre_factura}.xlsx'
    )
